...
def submit_data(column_panel):
        ...
        for widget in section.column_panel.get_components():
            ...
            if "text_box" == widget.__name__:
                ...
                if widget.label_mandatory.visible and not val.strip():
                    anvil.alert(
                        "Responses have not been submitted",
                ...
                for c in widget.column_panel.get_components():
                    # print(c)
                    if "check_box_other" == c.__name__ and c.check_box.checked:
                        val.append(c.text_box.text)

                    elif "check_box" == c.__name__ and c.checked:
                        val.append(c.text)
    ...
    thank_you_msg = anvil.server.call("submit_data", cols, data, anvil.get_url_hash())
    anvil.open_form("form.thank_you_msg", thank_you_msg)
...
def set_logic_target_ids(form_dict):
    ...
    target_logic_components = [
        form_dict[k] for k in form_dict if form_dict[k].tag.logic
    ]
...
def check_logic_for_visibility(target_id):
    ...
    for condition in logic["conditions"]:
        ...
        if source_comp.__name__ == "date":
            test_val = anvil.server.call_s(
                "str_to_date_obj", condition["value"], source_comp.date_picker.format
            )
        else:
    ...
    anvil.get_open_form().color_rows()
