...
def str_to_date_obj(date_str, date_format):
    ...
    # until strptime is implemented on the client (forcing to date until time is
...
def submit_data(cols, data, url_hash):
    ...
    df_new.columns = pd.io.parsers.base_parser.ParserBase(
        {"names": df_new.columns, "usecols": None}
    )._maybe_dedup_names(df_new.columns)
