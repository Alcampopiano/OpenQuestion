...
def get_json_schema():
    ...
    schema = anvil.http.request(
        "https://vega.github.io/schema/vega-lite/v5.json", json=True
    )
...
def data_to_spec(survey_row, cols, dataset_name):
    ...
    dataset = app_tables.forms.get(form_id=survey_row["form_id"])["reports"][
        "datasets"
    ][dataset_name]
        ...
        for node in tree:
            ...
            if node == key and type(tree[node]) is str:
                ...
                if pd.api.types.is_string_dtype(df[tree[key]]):
                    ...
                    # add column name to a list so that you can control the template
                    # placeholders
                    nominal_fields.append(tree[key]) if tree[
                        key
                    ] not in nominal_fields else nominal_fields
                    ...
                    quant_fields.append(tree[key]) if tree[
                        key
                    ] not in quant_fields else quant_fields
                    ...
                    temporal_fields.append(tree[key]) if tree[
                        key
                    ] not in temporal_fields else temporal_fields
...
def save_report(survey_dict, schema, specs, data_dicts):
    ...
    if not report_row:
        report_row = app_tables.reports.add_row(
            title=schema["title"],
        ...
        report_row.update(
            title=schema["title"], schema=schema, charts=specs, datasets=data_dicts
        )
    ...
    """
        ...
        """
                ...
                if data_name and data_values:
                    ...
                    html += gen_vega_vis_named_data(
                        widget_schema["id"], spec, data_name, data_values
                    )
